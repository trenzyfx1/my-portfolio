...
// Add an event listener to handle external link clicks
document.querySelectorAll('a').forEach(link => {
    link.addEventListener('click', function (e) {
      // Check if the link is external (not part of your domain)
      const isExternal = link.hostname !== window.location.hostname;
      
      if (isExternal) {
        // Prompt the user before leaving the site
        const confirmation = confirm("You are about to leave this site. Are you sure?");
        
        if (!confirmation) {
          e.preventDefault(); // Prevent the link from being followed if the user cancels
        }
      }
    });
  });
